...
The setting is saved in `~/.killsloprouter/plugin-sync.json`. Policy changes preserve the prior file as a backup. `--home DIR` supports an explicit user-home root; account homes must be existing, non-overlapping directories below that root. Uniqueness is checked after filesystem canonicalization, including case aliases on macOS. Account-home symlinks and executable fields in settings are rejected. Unknown sync options are rejected before discovery or mutation: an `--account-home` typo cannot silently enroll every account. A pre-existing `plugins` symlink is accepted only if it belongs to the current user and points directly to another enrolled account's real plugin directory. The link is checked before and after Codex calls and reported as `cache_shared_with`; KSR never creates or rewires it.
...
Mutating sync/configuration and installation commands share one exclusive lock. Installation acquires it before reading the effective policy or changing the canonical bundle, legacy shim or marketplace, and holds it through activation. Even `--no-activate` installation respects the lock; dry-run remains read-only. Lock ownership (parent identity, file identity and token bytes) is checked before and after account children and before persisting a sync receipt. Ownership loss blocks later children, prevents receipt persistence and preserves the replacement lock for inspection; earlier child effects are not rolled back. If a process crashes with the lock present, inspect `~/.killsloprouter/plugin-sync.lock` and confirm that the previous operation ended before explicitly moving that exact file to a backup path. The command never treats a PID alone as stale-lock authority or automatically deletes an existing lock.
