...
// Collect first so inventory checks can inspect every case without starting a
// child or browser. Both the historical entrypoint and the bounded E2E shards
// register the same callbacks, with their original options and assertions.
const cases = [];
function test(...args) {
  cases.push(args);
}

export const DESIGN_SHARD_COUNT = 8;

export function selectDesignTests({ shard = 0, shards = 1 } = {}) {
  if (!Number.isSafeInteger(shard) || !Number.isSafeInteger(shards) ||
    shard < 0 || shard >= shards || ![1, DESIGN_SHARD_COUNT].includes(shards)) {
    throw new Error("invalid KillSlopRouter design test shard selection");
  }
  const names = cases.map(([name]) => name);
  if (names.some((name) => typeof name !== "string" || !name.length) ||
    new Set(names).size !== names.length) {
    throw new Error("design test inventory requires unique named cases");
  }
  return cases.filter((_test, ordinal) => ordinal % shards === shard);
}

export function registerDesignTests(selection) {
  for (const args of selectDesignTests(selection)) nodeTest(...args);
}
